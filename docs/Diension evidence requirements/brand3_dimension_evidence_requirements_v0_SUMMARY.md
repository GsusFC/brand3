...
### 1. The Core Problem Is Not Evidence Packet Classification — It's Upstream Acquisition

Evidence Packet v0 is working correctly. It classifies evidence present in the snapshot. The failing dimensions (especially diferenciación) fail because the input snapshots **do not contain the right evidence type** — not because the packet is misclassifying evidence.
...
**Implication**: Fixing Evidence Packet will not fix diferenciación. Must fix upstream collection.
...
This is incorrect. Technical-only signals (contrast ratios, pixel alignment) are not coherencia evidence. But brand-level visual behavior IS part of messaging consistency.

**Fix**: Refine visual signal classification in Evidence Packet:
- **Block**: contrast ratios, pixel alignment, layout metrics.
- **Allow**: color palette consistency, typography consistency, logo usage patterns.
...
- If status = thin: flag to narrative engine; can score but mark as qualif  
- If status = blocked: abstain; do not score; do not render finding
- If status = insufficient_input: escalate (manual review / Deep Research)
...
**Percepción**: External perception. Requires 100% external (no owned self-description). Can abstain if zero external + high presencia (meaningful absence).
