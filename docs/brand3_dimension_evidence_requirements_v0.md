...
Evidence eligibility and dimension readiness are separate decisions:

- An evidence item may be eligible, while the dimension is still not ready.
- A dimension may contain evidence, while still requiring abstention.
- Evidence Packet v0 is diagnostically useful, but it is not prompt-ready.
- Evidence Packet v0 exposes upstream acquisition gaps, but it still has classification defects: empty-text URL evidence, coarse source roles, social/profile over-gating, and incomplete dimension coverage checks.

### Cross-Dimension Hard Rules

- URL without evidence text cannot count toward readiness.
- Technical/internal artifacts cannot make any dimension ready.
- Visual brand signals can support coherencia readiness, but cannot satisfy readiness alone.
- Visual technical metrics cannot support narrative readiness.
- Owned claims cannot count as external corroboration.
- Official social/profile evidence requires owned-surface link verification or review gate.
- Review-gated evidence cannot count toward ready status unless manually approved.
- Evidence Packet must output readiness separately from eligibility.
...
- **Brand-level visual consistency may support readiness** only when paired with owned textual/message surfaces.
...
**Disallowed**:
- Visual technical metrics (color counts, contrast ratios, whitespace ratio, layout stats, screenshot metrics) — these are technical/internal signals, not messaging evidence.
...
- Allowed as support: repeated logo usage, consistent named palette, consistent typography system, repeated brand-level layout language.
- Disallowed as support: color count, contrast ratio, whitespace ratio, screenshot metric, pixel/layout measurement.
- Visual brand signals cannot independently make coherencia ready.
...
**No, not as normal sentiment scoring.** Zero external perception can only be an observable absence when all of the following are true:

- search coverage is documented,
- presencia is ready/high,
- query/source scope is known,
- no external sentiment sources are found.

Otherwise percepcion must abstain or remain thin. Observable absence must not be rendered as sentiment.
...
```json
{
  ...
  "status": "ready | thin | blocked | abstain | review_required",
  "evidence_count": {
    "eligible": 0,
    "review_gated": 2,
    "blocked": 2,
...
## Implementation Contract For Next Evidence Packet Hardening

The next Evidence Packet hardening pass must implement a separate `dimension_readiness` object. It must not treat item-level eligibility as dimension readiness.

Required statuses:

- `ready`: evidence contract is satisfied.
- `thin`: usable evidence exists, but prose must qualify limits.
- `blocked`: evidence exists but is the wrong type or unsafe for the dimension.
- `abstain`: do not render score/finding because the dimension contract is unsatisfied.
- `review_required`: manual approval is required before prompt use.

Required reason codes:

- `empty_text_evidence_blocked`
- `technical_internal_only`
- `visual_technical_metric_blocked`
- `visual_brand_support_only`
- `owned_claim_without_external_corroboration`
- `official_profile_requires_link_verification`
- `review_gated_evidence_not_ready`
- `insufficient_comparative_evidence`
- `competitor_corpus_required`
- `observable_absence_conditions_not_met`

Required source-role subtypes:

- `owned_page`
- `owned_changelog`
- `owned_docs`
- `news_article`
- `encyclopedic_profile`
- `review_site`
- `release_aggregator`
- `code_repository`
- `code_mirror`
...
- Empty-text URL evidence is blocked from normal eligibility and cannot count toward readiness.
- Official social/profile candidates require owned-surface outbound-link verification or stay `review_required`.
- Diferenciacion must abstain and request competitor corpus when no direct comparative or category-distinctive external evidence exists.
- Percepcion observable absence may be rendered only when search coverage, high/ready presencia, query scope, and no external sentiment source are documented.
- The packet remains documentation/input-ordering infrastructure until the readiness contract passes multi-case validation.
...
2. **Add dimension coverage checks**: Each dimension outputs status (ready/thin/blocked/abstain/review_required).
