...
### 4. Visual Coherence Evidence Needs Support-Only Semantics (MEDIUM)
...
**Current readiness**: Evidence Packet v0 exposes important upstream gaps (competitor corpus, entity validation, social verification), but the Vercel test also shows it is not prompt-ready. It still has local classification defects: empty-text URL evidence, coarse source roles, social/profile over-gating, and missing dimension-level readiness output.
