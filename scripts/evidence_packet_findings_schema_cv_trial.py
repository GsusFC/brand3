#!/usr/bin/env python3
"""Lab-only multi-case trial for coherencia/vitalidad with the v0 findings schema."""
...
OUT_DIR = Path("examples/reports/evidence_packet_findings_schema_trial/coherencia_vitalidad_v0")
...
def main() -> None:
    ...
    cost_observation: dict[str, Any] = {
        ...
        "notes": ["Single batch CV-dim trial; provider usage metadata not exposed by LLMAnalyzer."],
    }

    if args.execute and manifest["requests"]:
        analyzer = LLMAnalyzer(model=LLM_PREMIUM_MODEL)
        for request in manifest["requests"]:
            raw_outputs["results"].append(_call(analyzer, request))
    ...
    for target in TARGETS:
        ...
        user_payload = {
            ...
            "prompt_rules": [
                ...
                "If evidence is narrow, produce one narrow finding.",
                "Return JSON only.",
            ],
            "included_evidence": [
                {
...
def _summary(data: Any, allowed_urls: list[str], dimension: str) -> dict[str, Any]:
    ...
    return {
        ...
        "limits_field_present": all(bool(str(item.get("limits") or "").strip()) for item in findings if isinstance(item, dict)) if findings else False,
...
def _comparison(manifest: dict[str, Any], raw_outputs: dict[str, Any]) -> dict[str, Any]:
    ...
    per_case = {
        f"{result['case_id']}::{result['dimension']}": {
...
def _thin_case_qualified(per_case: dict[str, Any]) -> bool:
    key = "vercel::coherencia"
    if key not in per_case:
        return False
    summary = per_case[key]["summary"]
    return summary["finding_count"] <= 2 and len(summary["risky_terms_outside_limits"]) == 0
...
def _write_notes(manifest: dict[str, Any], comparison: dict[str, Any], executed: bool) -> None:
    (OUT_DIR / "trial_notes.md").write_text(
        f"""# Evidence Packet Findings Schema CV Trial
...
Focus dimensions: coherencia + vitalidad.
