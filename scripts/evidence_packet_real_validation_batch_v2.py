#!/usr/bin/env python3
"""Real validation batch v2 with segmented case matrix and backup resolution.
...
OUT_DIR = Path("examples/reports/evidence_packet_real_validation_batch_v2")
PACKET_DIR = Path("examples/reports/evidence_packet")
CANDIDATE_DIR = Path("examples/reports/evidence_packet_prompt_input")
REVIEW_MD = Path("docs/brand3_evidence_packet_real_validation_batch_v2_review.md")
REVIEW_JSON = Path("docs/brand3_evidence_packet_real_validation_batch_v2_review.json")
...
CASE_MATRIX = {
    "group_a": {
        "name": "High evidence / low ambiguity",
        "primary": [
        ...
        "backups": [
            {"case_id": "netlify", "brand_name": "Netlify", "canonical_url": "https://netlify.com"},
            {"case_id": "heroku", "brand_name": "Heroku", "canonical_url": "https://heroku.com"},
            {"case_id": "modal", "brand_name": "Modal", "canonical_url": "https://modal.com"},
            {"case_id": "linear", "brand_name": "Linear", "canonical_url": "https://linear.app"},
            {"case_id": "asana", "brand_name": "Asana", "canonical_url": "https://asana.com"},
        ],
    },
    "group_b": {
        "name": "Strong visual identity / weak external grounding",
        "primary": [
        ...
        "backups": [
            {"case_id": "hex_studio", "brand_name": "HEX Studio", "canonical_url": None},
            {"case_id": "metabrand", "brand_name": "Metabrand", "canonical_url": None},
            {"case_id": "spectra", "brand_name": "Spectra", "canonical_url": None},
            {"case_id": "8b", "brand_name": "8B", "canonical_url": None},
        ],
    },
    "group_c": {
        "name": "Contradiction-heavy / multi-signal tension",
        "primary": [
            {"case_id": "ag2", "brand_name": "AG2", "canonical_url": "https://www.ag2.ai"},
            {"case_id": "anyforge", "brand_name": "AnyForge", "canonical_url": "https://anyforge.ai"},
            {"case_id": "namzu", "brand_name": "Namzu", "canonical_url": None},
            {"case_id": "cont3xt", "brand_name": "Cont3xt", "canonical_url": "https://cont3xt.dev"},
            {"case_id": "agentspan", "brand_name": "Agentspan", "canonical_url": "https://agentspan.ai"},
        ],
        "backups": [
            {"case_id": "delimit", "brand_name": "Delimit", "canonical_url": None},
            {"case_id": "openclaw", "brand_name": "OpenClaw", "canonical_url": None},
            {"case_id": "cursor", "brand_name": "Cursor", "canonical_url": "https://cursor.com"},
            {"case_id": "lovable", "brand_name": "Lovable", "canonical_url": "https://lovable.dev"},
            {"case_id": "anthropic", "brand_name": "Anthropic", "canonical_url": "https://www.anthropic.com"},
        ],
    },
}
...
def _build_request_manifest(snapshot_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for case in snapshot_manifest.get("cases") or []:
        ...
        request, skip = build_lab_request(
            ...
            task_label=f"Generate bounded findings for {case_id}/{dimension} in real validation batch v2.",
    ...
    for control in BLOCKED_CONTROLS:
        ...
        request, skip = build_lab_request(
            ...
            model=LLM_PREMIUM_MODEL,
            task_label=f"Control check for {case_id}/{dimension} in batch v2.",
            readiness_status=str(dim_payload["readiness_status"]),
            prompt_constraints=list(dim_payload.get("prompt_constraints") or []),
            evidence=list(dim_payload.get("evidence") or []),
            max_tokens=1800,
...
def _build_comparison(
    ...
    group_level_flags = {
        ...
        "group_b_conservative_thin_or_review": (
            (group_b.get("readiness_distribution", {}).get("ready", 0) <= 2)
            and (group_b.get("readiness_distribution", {}).get("thin", 0) + group_b.get("readiness_distribution", {}).get("review_required", 0) >= 1)
        ),
        "group_c_strong_abstain_or_blocked_discipline": (
            group_c.get("readiness_distribution", {}).get("blocked", 0)
            + group_c.get("readiness_distribution", {}).get("abstain", 0)
            >= max(1, group_c.get("selected_count", 0) - 1)
        ),
    }
...
def _write_notes(
    ...
    lines: list[str] = []
    lines.append("# Evidence Packet Real Validation Batch v2")
    lines.append("")
...
def _write_review(
    ...
    review_json = {
        "review": "brand3_evidence_packet_real_validation_batch_v2",
    ...
    md_lines.extend(
        [
            ...
            "## Recommendation",
            (
                "- Promote v2 dry-path as default lab benchmark for pre-runtime candidate integration."
                if overall_pass
                else "- Keep v2 experimental. Resolve failure classes before promotion."
            ),
        ]
    )
...
def _failure_classes(comparison: dict[str, Any], case_matrix_payload: dict[str, Any]) -> list[str]:
    ...
    if case_matrix_payload.get("unresolved_total", 0) > 0:
