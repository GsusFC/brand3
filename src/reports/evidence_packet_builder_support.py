"""Builder helpers for the offline Evidence Packet v0 builder."""
...
def packet_identity(snapshot: dict) -> dict[str, str]:
    run = snapshot.get("run") or {}
    audit_url = str(run.get("url") or "").strip()
    audit_host = _host_impl(audit_url)
    audit_root = _root_domain_impl(audit_host)
...
def empty_packet(*, case_id: str, audit_url: str, audit_host: str, audit_root: str) -> dict:
    return {
        "version": VERSION,
        "case_id": case_id,
        "audit_url": audit_url,
...
def process_candidate(
    *,
    packet: dict,
    classified: dict,
    seen_ambiguities: set[tuple[str, str]],
    seen_reviews: set[tuple[str, str]],
    seen_missing: set[tuple[str, str]],
) -> None:
    dimension = classified.get("dimension") or "unknown"
    packet["dimension_evidence_inputs"].setdefault(dimension, []).append(dimension_input(classified))

    source_class = classified["source_class"]
    eligibility = classified["eligibility"]
    entry = public_entry(classified)

    if source_class == "audited_surface":
        packet["audited_surface"]["evidence"].append(entry)
        if _looks_like_owned_claim_impl(classified):
...
def finalize_metadata_counts(packet: dict) -> None:
...
def dimension_input(item: dict) -> dict:
    return {
        "text": item.get("text") or "",
        "url": item.get("url") or "",
        "feature_name": item.get("feature_name") or "",
