"""Private helpers for the evidence material diff LLM shadow."""
...
from src.research.evidence_semantic_llm import _latest_llm_failure, _structured_transport, _truncate
...
def _material_diff_candidates(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in work_orders:
        if not isinstance(item, dict) or str(item.get("next_action") or "") != "manual_audit_projected_material_changes":
            continue
...
def _normalize_response(raw: Any, *, expected_ids: set[str]) -> list[dict[str, Any]] | None:
    ...
    for row in rows:
        ...
        if work_order_id not in expected_ids or work_order_id in seen:
            return None
        if verdict not in {"approve_vnext_material", "send_back_for_evidence_correction", "keep_manual_review"} or material_risk not in {"low", "medium", "high"}:
            return None
        if entity_fit not in {"strong", "partial", "missing", "wrong_entity"} or source_trust not in {"strong", "acceptable", "weak", "untrusted", "missing"}:
            return None
...
def _latest_failure(analyzer: Any) -> dict[str, Any]:
    return _latest_llm_failure(analyzer)


def _transport(analyzer: Any) -> str:
    return _structured_transport(analyzer)
