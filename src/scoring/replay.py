...
def _resolve_calibration_profile_callable() -> Any:
    """Return a monkeypatchable calibration profile resolver from the facade module."""
    try:
        import importlib

        module = importlib.import_module("src.scoring.replay")
        override = getattr(module, "get_calibration_profile", None)
        if callable(override) and override is not get_calibration_profile:
            return override
    except Exception:
...
def build_score_replay_audit(
    ...
    *,
    ...
    current_profile_config = _resolve_calibration_profile_callable()(calibration_profile)
