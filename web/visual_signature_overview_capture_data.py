"""Screenshot preview and visual evidence helpers for the Visual Signature web lab."""
...
def build_screenshot_preview_model_for_lang(filename: str, lang: str = "es") -> dict[str, Any] | None:
    ...
    selected_brand, selected_label = variant_from_filename(selected_path)
    evidence = visual_evidence_model()
    selected_item = next((item for item in evidence["items"] if item.get("capture_id") == selected_brand), None)
    if selected_item is None:
        selected_item = _fallback_preview_item(selected_brand)

    selected_variant = next(
        (dict(variant) for variant in selected_item["variants"] if variant["filename"] == selected_path.name),
        screenshot_variant_payload(selected_label, selected_path),
    )
        ...
        items.append(
            {
                ...
                "variants": screenshot_variants(row, screenshots_dir=screenshots_dir),
            }
        )
    ...
    if not items and screenshots_dir.exists():
        ...
        for path in sorted(screenshots_dir.glob("*")):
            if path.suffix.lower() not in {".png", ".jpg", ".jpeg", ".webp"}:
                continue
            brand, label = variant_from_filename(path)
            grouped.setdefault(brand, _fallback_preview_item(brand))["variants"].append(screenshot_variant_payload(label, path))
...
def screenshot_variants(row: dict[str, Any], *, screenshots_dir: Path) -> list[dict[str, Any]]:
    ...
    return [screenshot_variant_payload(label, Path(path)) for label, path in candidates if path]


def screenshot_variant_payload(label: str, path: Path) -> dict[str, Any]:
...
def variant_from_filename(path: Path) -> tuple[str, str]:
    stem = path.stem
...
def _fallback_preview_item(selected_brand: str) -> dict[str, Any]:
    root = visual_signature_root()
    return {
        ...
        "evidence_notes": [],
        "variants": [
            screenshot_variant_payload("raw viewport", root / "screenshots" / f"{selected_brand}.png"),
            screenshot_variant_payload("clean attempt", root / "screenshots" / f"{selected_brand}.clean-attempt.png"),
            screenshot_variant_payload("full page", root / "screenshots" / f"{selected_brand}.full-page.png"),
        ],
    }
